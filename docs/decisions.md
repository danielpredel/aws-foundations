...
CloudWatch Logs was selected to centralize application logs generated by the EC2-hosted FastAPI service.

The CloudWatch Agent collects logs from `systemd`/`journald`, while Terraform manages the log group and IAM permissions required to publish log events.

CloudWatch metrics and alarms are outside the scope of this project and will be introduced in a later project.

## Terraform Modules

The Terraform configuration is divided into reusable modules for networking,
IAM, S3, EC2 and CloudWatch.
